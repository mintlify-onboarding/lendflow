...
- Application Fully Submitted (a new application/deal has been created)
...
To retrieve application data for a single application, use the following endpoint: [Lendflow-External](https://docs.lendflow.com/api-docs/reference/retrieveanapplicationusingworkflowbuilder).  
...
[block:html]
{
  "html": "<video controls\n    src=\"https://lendflow-documentation.s3.us-west-2.amazonaws.com/GoldenPath_RetrieveDocument.mov\"\n    width=\"800\">\n\n\n</video>"
}
[/block]
...
To run Data Orchestration manually on a deal via the API, use this endpoint: [Lendflow-External](https://docs.lendflow.com/api-docs/reference/rundataorchestrationforanapplication). Enter the Data Orchestration template id in the template_id field and the application uuid in the application_id field.
...
To create new offers in Lendflow via the API, use the following endpoint [Lendflow-External](https://docs.lendflow.com/api-docs/reference/createaselffundedoffer). The `application_id` and `product_id` are required fields. The accepted `product_ids` are: 
...
To view the available fields to submit, refer to this guide: [Lendflow-External](https://docs.lendflow.com/api-docs/docs/offer-field-requirements-by-products)
...
If Lendflow is leading the communication with the applicant, Lendflow will notify users on the status of an offer via the following webhooks: Offer Accepted, Offer Sent, Offer Declined, Offer Retracted. In this webhook notification, Lendflow will send the `offer_id`. To locate the offer details using this `offer_id`, use the following endpoint [Lendflow-External](https://docs.lendflow.com/api-docs/reference/applicationoffers) to return the full offer details.

## Updating the Offer Status

If client users are leading the communication with the applicant at the offer stage, then this endpoint can be used to update the offer status [Lendflow-External](https://docs.lendflow.com/api-docs/reference/updateofferstatus).

## Rejecting an Application

To reject an application via the API, use the following endpoint [Lendflow-External](https://docs.lendflow.com/api-docs/reference/declineapplication). Specify the application id of the deal to be rejected and submit a reason and sub-reason. These reason fields are freeform and accept any string values.